...
public class JwtConfiguration extends WebSecurityConfigurerAdapter {
	...
	/*
	 * Esses são os endpoints da API que nao irão exigir autenticação para serem acessadas
	 */
	@Override
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().addFilterAfter(new JwtSecurity(), UsernamePasswordAuthenticationFilter.class)
				.authorizeRequests().antMatchers(HttpMethod.POST, "/api/registration").permitAll()
